...
import { MapperProductToProductPay } from "../../productsPay/Application/MapperProductToProductPay";
import { PaymentService } from "../../../../libs/mercadopago";
import { CreatePay } from "../Application/CreatePay";
// import { AddPayId } from "../../productsPay/Application/AddPayId";
import { ProductPayService } from "../../productsPay/Domain/ProductPayService";
import { Pay } from "../Domain/pay";
import { CreateProductsPay } from "../../productsPay/Application/CreateProductsPay";
...
export class PayController {
    ...
    create = (req: Request, res: Response,) => {
        ...
        // let payerId = "01465906-f64b-47fe-aea4-37bb4176bfb4"
        const ids = items.map((item: any) => item.id)
        let payerReq: Payer
        let productsValidate:any[]
        let payReq : Pay 
        console.log({payerId,ids, items})
        return new GetPayerById(this.payerService)
            ...
            .then(pay => {
                payReq = pay
                return new MapperProductToProductPay()
                    .execute(productsValidate, "")
            })
            .then(productPay=>{
                return new CreateProductsPay(this.productPayService)
                    .execute(productPay, payReq.id)
            })
            .then(productsPay => {
